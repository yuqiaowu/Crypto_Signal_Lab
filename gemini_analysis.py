...
6. 量能分析以“前一天”为准：因为我们执行命令时当天尚未结束，当日成交量相对于全天必然偏小。若涉及最近一天，请优先参考 `volume_ratio_ma20_effective` 字段（该字段已将最后一日的“当日量能占比”替换为前一日的占比），避免误判。
7. 请判断是否“有效站上/跌破关键均线”，并给出依据：默认关注 MA20/MA60/MA120/MA180；“有效”的定义为价格至少连续3天在均线之上/之下（可参考 `ma_status.*.stood_above_3` / `ma_status.*.fell_below_3`），同时量能不低于均量（以 `volume_ratio_ma20_effective` 为准；≥1.0 为基本支持，≥1.5 为较强支持）。
请输出：
